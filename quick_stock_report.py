...
def generate_recommendation(results: dict, processor) -> str:
    ...
    market = results.get('market_sensitivity', {})
    ...
    if desc['sharpe_ratio'] > 1.0:
        score += 2
        reasons.append("Strong risk-adjusted returns (Sharpe > 1.0)")
    elif desc['sharpe_ratio'] > 0.5:
        score += 1
        reasons.append("Moderate risk-adjusted returns (Sharpe > 0.5)")
    ...
    if abs(risk['max_drawdown']) < 0.20:
        score += 1
        reasons.append("Limited drawdown risk (<20%)")
    
    if market.get('alpha_annual', 0) > 0:
        score += 1
        reasons.append("Positive alpha vs market")
    
    if desc['positive_days_pct'] > 55:
        score += 1
        reasons.append(f"High win rate ({desc['positive_days_pct']:.1f}%)")
    ...
    if abs(risk['max_drawdown']) > 0.50:
        score -= 2
        reasons.append("Severe drawdown risk (>50%)")
    
    if desc['mean_annual_return'] < 0:
        score -= 2
        reasons.append("Negative annual returns")
    
    # Ensure we have reasons
    if not reasons:
        reasons.append("Insufficient data for comprehensive assessment")
...
def create_summary_sheet(analysis: dict) -> pd.DataFrame:
    ...
    desc = results['descriptive_stats']
    risk = results['risk_metrics']
    market = results.get('market_sensitivity', {})  # Make optional
    ...
    # === MARKET SENSITIVITY === (Only if available)
    if market and any(v is not None and v != 0 for v in market.values()):
